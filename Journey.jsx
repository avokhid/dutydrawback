...
// ─────────────────────────────────────────────────────────────────────────
// Duty Drawback — Full Journey
//
// Strings the pieces into one continuous flow so you can click through the
// whole estimator path end to end:
//
//   Upload  →  Extract (live feed)  →  Results  →  Run (live feed)  →  Reviewer
//
// Each stage is a real component; this wrapper just owns the step state and the
// hand-offs. In production the same flow holds, with each step backed by its API
// endpoint (upload+extract, run-stream, matches, corrections).
//
// Self-contained: paste UploadScreen, a compact Extract/Run feed, a Results
// summary, and a compact Reviewer card inline so the whole journey renders from
// one file for preview. The full standalone components (UploadScreen.jsx,
// ActivityFeed.jsx, App.jsx, EstimateReport.jsx) are the production versions.
// ─────────────────────────────────────────────────────────────────────────

const C = {
  bg: "#10110F", panel: "#181A17", panel2: "#1F221D", line: "#2C302A",
  lineSoft: "#23261F", ink: "#EDEFE9", inkSoft: "#A7AC9E", inkFaint: "#6E7567",
  ok: "#7FB069", okBg: "#1A2417", busy: "#6BA8C9", warn: "#D8A24A", warnBg: "#241E12",
  err: "#C16E5A", accent: "#6BA8C9", accentBg: "#13202722",
};
const mono = "'JetBrains Mono','SF Mono',ui-monospace,monospace";
const sans = "'Inter',-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif";

const STEPS = ["Upload", "Read", "Confirm", "Estimate", "Review"];
      ...
      {STEPS.map((s, i) => (
        <React.Fragment key={s}>
          <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
            <div style={{
              width: 22, height: 22, borderRadius: "50%", fontFamily: mono, fontSize: 11,
              display: "flex", alignItems: "center", justifyContent: "center",
              background: i < current ? C.ok : i === current ? C.accent : C.panel2,
              color: i <= current ? C.bg : C.inkFaint,
              border: i > current ? `1px solid ${C.line}` : "none",
            }}>{i < current ? "✓" : i + 1}</div>
            <span style={{ fontFamily: sans, fontSize: 12.5, color: i === current ? C.ink : C.inkFaint, fontWeight: i === current ? 500 : 400 }}>{s}</span>
          </div>
          {i < STEPS.length - 1 && (
            <div style={{ width: 24, height: 1, background: i < current ? C.ok : C.line, margin: "0 10px" }} />
          )}
        </React.Fragment>
      ))}
    </div>
...
function FeedRow({ label, status, summary, sub }) {
  const col = status === "complete" ? C.ok : status === "running" ? C.busy : C.inkSoft;
  return (
    <div style={{ display: "flex", gap: 12, padding: "13px 16px", borderTop: `1px solid ${C.lineSoft}`,
      background: status === "complete" ? C.okBg : "transparent" }}>
      ...
      <div style={{ flex: 1 }}>
        ...
        {sub && <div style={{ fontFamily: sans, fontSize: 12, color: C.warn, marginTop: 4 }}>{sub}</div>}
      </div>
    </div>
  );
}

function MiniFeed({ title, rows, footer }) {
  return (
...
function Btn({ children, onClick, kind = "primary" }) {
  const styles = {
    ...
    go: { color: C.bg, background: C.ok, border: "none" },
    ghost: { color: C.ink, background: "transparent", border: `1px solid ${C.line}` },
  }[kind];
  return (
    <button onClick={onClick} style={{ ...styles, fontFamily: sans, fontSize: 13.5, fontWeight: 500,
      borderRadius: 8, padding: "10px 18px", cursor: "pointer", marginTop: 18 }}>{children}</button>
  );
}
...
export default function Journey() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, padding: "40px 24px 80px", fontFamily: sans }}>
      <style>{`*{box-sizing:border-box} button:hover{filter:brightness(1.1)}
      ...
      <div style={{ maxWidth: 680, margin: "0 auto" }}>
        ...
        {step === 0 && (
          <div>
            ...
            <div style={{ border: `1px solid ${C.line}`, borderRadius: 12, overflow: "hidden", background: C.panel }}>
              {[["entry_summary_7501.pdf", "import"], ["export_bol_77.pdf", "export"]].map(([n, k], i) => (
        ...
        {step === 1 && (
          <MiniFeed title="Reading your documents"
            rows={[
        ...
        {step === 2 && (
          <div style={{ maxWidth: 600 }}>
            ...
            <div style={{ fontFamily: sans, fontSize: 12.5, color: C.ok, marginTop: 12 }}>No problems found — your documents are clean.</div>
            <Btn kind="go" onClick={() => setStep(3)}>Run estimate →</Btn>
          </div>
        )}

        {step === 3 && (
          <MiniFeed title="Estimating your recovery"
            rows={[
              ...
              { label: "Estimating your recovery", status: "complete", summary: "You could recover about $2,970.00." },
            ]}
            footer={<Btn onClick={() => setStep(4)}>Review the uncertain pairing →</Btn>} />
        )}
          ...
          <div style={{ maxWidth: 620 }}>
            ...
            <div style={{ border: `1px solid ${C.line}`, borderRadius: 12, overflow: "hidden", background: C.panel }}>
              {[["HTS", "8471.30.0100", "8471.30.0100", true],
                ...
                ["Value", "$124,000", "$131,400", true]].map(([f, imp, exp, ok], i) => (
                <div key={f} style={{ display: "grid", gridTemplateColumns: "120px 1fr 1fr", padding: "11px 16px", borderTop: i > 0 ? `1px solid ${C.lineSoft}` : "none", background: ok ? "transparent" : C.warnBg }}>
                  <span style={{ fontFamily: sans, fontSize: 12.5, color: ok ? C.inkSoft : C.warn, fontWeight: ok ? 400 : 500 }}>{f}{!ok && " ⚠"}</span>
                  <span style={{ fontFamily: mono, fontSize: 12.5, color: C.ink }}>{imp}</span>
                  <span style={{ fontFamily: mono, fontSize: 12.5, color: C.ink }}>{exp} {ok && <span style={{ color: C.ok }}>✓</span>}</span>
                </div>
              ))}
            </div>
