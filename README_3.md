...
**33 tests pass.** The UNTESTED file (and the API parts of verification.py) need
...
```bash
pip install pydantic pytest anthropic --break-system-packages
...
1. Package the modules (`src/drawback/`, `__init__.py`, relative imports). Keep
   all 33 tests green — they are the contract; a refactor that changes expected
